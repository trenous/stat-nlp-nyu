...
public class MaximumEntropyClassifier<I, F, L> implements
	...
	public static class ObjectiveFunction<F, L> implements
		...
		private Pair<Double, double[]> calculate(double[] x) {
			...
			//  Array to save the probabilities of the data for reuse
    		double[][] probabilities = new double[data.length][encoding.getNumLabels()];
    		// The L2 norm of the weights vector is needed for the penalty term
    		double L2Norm = 0.0;
    		double penaltyWeight = 0;
    		if (sigma != 0)
    			penaltyWeight =  0.5*(1/Math.pow(sigma,2)); 
    		int dataIndex = 0;
    		// Iterate over observed data to compute the objective
            for (EncodedDatum datum : this.data) {
            	probabilities[dataIndex] = getLogProbabilities(datum, x, encoding, indexLinearizer);
            	// Subtract the log probability from the objective
            	objective -= probabilities[dataIndex][datum.getLabelIndex()];
            	// Turn the log probability into a real probability for the computation of the derivative
            	for (int i = 0 ; i < encoding.getNumLabels() ; i++) 
            		probabilities[dataIndex][i] = Math.exp(probabilities[dataIndex][i]);
            	dataIndex++;
            }              
                        
            // Iterate over each element of the weight vector
            for (int i = 0; i < derivatives.length; i++) {
            	// Add the square to the L2 norm
            	L2Norm += Math.pow(x[i],2);
            	// add the penalty term to the derivative
            	 derivatives[i] += 2* x[i]* penaltyWeight;
               	...
               	for (EncodedDatum datum : this.data) {
               		...
               		int featureIndex = 0;
               		int labelIndex = 0;
               		// Add the weighted feature counts to the derivative
               		for (int knownFeature = 0 ; knownFeature < datum.getNumActiveFeatures() ; knownFeature++) {
               				featureCount = datum.getFeatureCount(knownFeature);
               				featureIndex = datum.getFeatureIndex(knownFeature);
               				labelIndex = datum.getLabelIndex();
               				// Add the count to w[label*feature]
               				derivatives[indexLinearizer.getLinearIndex(featureIndex, labelIndex)] -= featureCount;
               				for (int label = 0; label < encoding.getNumLabels() ; label++)
               					derivatives[indexLinearizer.getLinearIndex(featureIndex, label)] += probabilities[dataIndex][label] * featureCount;
               		}
               		dataIndex++;
            	}
               	
            // Add the penalty term to the objective
            objective += penaltyWeight * L2Norm;
	...
	private static <F, L> double[] getLogProbabilities(EncodedDatum datum,
			...
			IndexLinearizer indexLinearizer) {
		double[] labelProbabilities = new double[encoding.getNumLabels()]; 
		double normalizer = 0;
		// Iterate over each label and compute the value of the log probability
		for (int i = 0; i < encoding.getNumLabels(); i++) {
			labelProbabilities[i] = 0;
			// for each label, iterate over each active feature to compute the weighted sum w^T f(x)
			for (int j=0; j < datum.getNumActiveFeatures() ; j++) {
				int featureIndex = indexLinearizer.getLinearIndex(datum.getFeatureIndex(j), i);
				double featureCount = datum.getFeatureCount(j);
				labelProbabilities[i] +=  weights[featureIndex] * featureCount;
			}
	        normalizer += Math.exp(labelProbabilities[i]);
		}
		normalizer = Math.log(normalizer);
		// normalize log probabilities
		for (int i = 0; i < labelProbabilities.length ; i++) {
			labelProbabilities[i] -= normalizer;
		}
